...
function createFigure(x = 0, y = 0) {
    ...
    figure.on('pointerdown', (ev) => {
        ev.stopPropagation();
        destroyFigureHandler(figure);
    }, this);
...
function destroyFigureHandler(figure) {
    app.stage.removeChild(figure);
    figures.splice(figures.findIndex(item => item._boundsID === figure._boundsID && item._cachedTint === figure._cachedTint), 1);
    document.querySelector('#number-of-shapes').innerHTML = figures.length;
}
